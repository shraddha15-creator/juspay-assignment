...
function PreviewArea({ character, add_character }) {
	...
	return (
		...
		>
			<div className="flex justify-between mb-10">
				<div className=" text-xs bg-yellow-100 border border-yellow-500 h-6 py-4 px-2 rounded shadow-lg flex text-center items-center justify-center ">
					<button
						variant="contained"
						color="secondary"
					...
					>
						<AddIcon className="text-yellow-500 text-xs" />
						Add Sprite 🐱
					</button>
				</div>
			</div>
					...
					return (
						...
						>
							<div id={`${x.id}-div`} className="character">
								<div
									className="hidden rounded-3xl p-1 px-3 ml-3 bg-pink-200 border border-pink-500 shadow-md text-xs mb-2 w-auto whitespace-nowrap"
									id={x.id + "-message-box"}
								></div>
								<div
